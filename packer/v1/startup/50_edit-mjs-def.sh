...
if [[ ${NODE_TYPE} == 'HEADNODE' ]]; then
    # Set autoscaling_flag based on ENABLE_AUTOSCALING and MATLAB_RELEASE
    ...
    jq --argjson flag "$autoscaling_flag" '.config.autoscaling_enabled=$flag' ${CLUSTER_MANAGEMENT_DATA_FILE} > temp.json && mv temp.json ${CLUSTER_MANAGEMENT_DATA_FILE}
fi
