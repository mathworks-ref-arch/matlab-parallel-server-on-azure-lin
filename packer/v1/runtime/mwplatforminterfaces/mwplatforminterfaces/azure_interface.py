...
class AzureInterface(AbstractCloudInterface):
    ...
    def __init__(
        self, custom_dns_suffix: str = None, 
        use_private_ip_mapping: str = None,
    ) -> None:
    ...
    def set_headnode_tags(self, tags_to_update: dict[str, str]) -> bool:
        """Update tags on the headnode resource.
        ...
        tag_patch_resource = TagsPatchResource(
            operation = "Merge",
            properties = { "tags" : tags_to_update }
        )
        try:
    ...
    def is_spot_instance_marked_for_removal() -> bool:
        ...
        if events_data and "Events" in events_data:
            for event in events_data["Events"]:
                if event["EventType"] == "Preempt" and event["ResourceType"] == "VirtualMachine":
    ...
    def _get_azure_vm_scheduled_events() -> Any:
        ...
        try:
            ...
            print(f"Request failed while getting azure VM scheduled events from metadata endpoint: {e}")
            return None
