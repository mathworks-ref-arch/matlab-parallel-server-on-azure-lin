...
def backup_policy(
    ...
    if termination_policy == "":
        ...
        policy_reset = cloud_interface.set_headnode_tags(
            { CLUSTER_TERMINATION_TAG: termination_policy }
        )
        if not policy_reset:
            print(
...
def initialize_cluster_after_reboot(
    ...
    initial_desired_capacity = (
        cluster_management_interface.cluster_management_config[INITIAL_DESIRED_CAPACITY]
    )
    print(
