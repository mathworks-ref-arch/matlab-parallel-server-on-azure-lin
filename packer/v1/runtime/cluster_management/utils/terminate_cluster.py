    ...
    if not cluster_issue:
        ...
        if cluster_scaled_to_zero:
            ...
            if not mjs_stopped or not jobmanager_stopped:
                ...
                print(
                    f"> Resetting the cluster termination policy to the initial choice: {initial_termination_policy} and setting mw-state tag to down..."
                )
                tags_updated = cloud_interface.set_headnode_tags(
                    {
                        CLUSTER_STATUS_TAG: "down",
                        CLUSTER_TERMINATION_TAG: initial_termination_policy,
                    }
                )
                if tags_updated:
