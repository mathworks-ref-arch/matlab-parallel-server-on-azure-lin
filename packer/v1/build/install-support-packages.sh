...
if [[ ! -n "$SPKGS" ]]; then
 echo "No support packages to install."
 exit 0
fi
...
# Run mpm to install MATLAB support packages in the SPKGS variable
# into the target location. The mpm installation is deleted afterwards.
# The SPKGS variable should be a space separated list of MATLAB support packages, with no surrounding quotes.
