...
This section shows how to build the latest MATLAB VM image in your Azure account. 

Pull the source code and navigate to the Packer folder.
```bash
git clone https://github.com/mathworks-ref-arch/matlab-parallel-server-on-azure-lin.git
cd parallel-server-azure-linux-refarch/packer/v1
```
...
| CLIENT_ID           | *unset* | Client ID of Azure service principal. For more information on obtaining Azure service principal credentials, see [Create Azure credentials](https://learn.microsoft.com/azure/virtual-machines/linux/build-image-with-packer#create-azure-credentials). |
...
| AZURE_TAGS          | {Name="Packer Build", Build="MATLAB Parallel Server", Type="Linux"} | Tags applied by Packer to each deployed resource. |
| USER_ASSIGNED_MANAGED_IDENTITIES | *unset* | User-assigned managed identities to attach to the Packer VM during the build. |
| IMAGE_OFFER         | *unset* | Azure marketplace image offer to use as the base image for the build. |
| IMAGE_PUBLISHER     | *unset* | Azure marketplace image publisher for the base image. |
| IMAGE_SKU           | *unset* | Azure marketplace image SKU for the base image. |
| VM_SIZE             | *unset* | Azure VM size to use for the Packer build instance. |
...
If unspecified, Packer will install MATLAB, MATLAB Parallel Server&trade;, and all the available toolboxes. Use the Packer build-time variable POLYSPACE_PRODUCTS to specify the list of Polyspace&reg; products you want to install on the machine image. If unspecified, Packer will install Polyspace Bug Finder&trade; Server&trade; and Polyspace Code Prover&trade; Server&trade;.
To see the default toolboxes, go to the
...
```bash
packer build -var="PRODUCTS=MATLAB Deep_Learning_Toolbox" build-azure-parallel-server.pkr.hcl
```

Packer installs products using MATLAB Package Manager. For more information, see [MATLAB Package Manager](https://github.com/mathworks-ref-arch/matlab-dockerfile/blob/main/MPM.md). 

### **Customize MATLAB Parallel Server Release to Install**
By default, the Packer build uses the latest MATLAB Parallel Server release. To install an earlier MATLAB Parallel Server release, use one of the variable definition files in the [release-config](./release-config) folder. Note that although the files are available for MATLAB R2022b and later, MathWorks recommends using MATLAB R2023a or later. This is because the Packer builds for earlier releases use Ubuntu&reg; 20.04, which is no longer receiving security updates.
...
For example, install MATLAB R2023a and all available toolboxes.
```bash
packer build -var-file="release-config/R2023a.pkrvars.hcl" build-azure-parallel-server.pkr.hcl
```
Command line arguments can also be combined. For example, install only MATLAB R2023a, MATLAB Parallel Server, and Parallel Computing Toolbox&trade;.
```bash
packer build -var-file="release-config/R2023a.pkrvars.hcl" -var="PRODUCTS=MATLAB MATLAB_Parallel_Server Parallel_Computing_Toolbox" build-azure-parallel-server.pkr.hcl
```
...
To specify a MATLAB Parallel Server release using a variable definition file, modify the variable definition file
...
[Packer template](https://github.com/mathworks-ref-arch/matlab-parallel-server-on-azure-lin/blob/master/packer/v1/build-azure-parallel-server.pkr.hcl).
...
To deploy the custom machine image, see [Build and Deploy Your Own Machine Image](https://github.com/mathworks-ref-arch/matlab-parallel-server-on-azure-lin/blob/master/README.md#build-and-deploy-your-own-machine-image).
